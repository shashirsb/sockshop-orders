		...
		try {
			...
			try {
				...
				while (c.hasNext()) {
					...
					orders.shipment = Shipment.builder().carrier(_shipment.get("carrier").toString()).trackingNumber(_shipment.get("trackingNumber").toString()).deliveryDate(LocalDate.parse(_localdate.get("deliveryDate").toString(), DateTimeFormatter.ofPattern("yyyy-MM-d"))).build();
